...
EOS
  # extensions の下に追加
  sed -i -e "/^extensions = \[/r tmp.py" $output_dir/conf.py
...
add_pages()
{
  ...
  find ./ -maxdepth 1 -type f -name "*.md" -exec cp -f {} $output_dir/pages/ \;
  
  echo '   ' > tmp.txt
  find ./ -maxdepth 1 -type f -name "*.rst" -o -name "*.md" \
    | sort | sed 's|^.|   /pages|' >> tmp.txt
