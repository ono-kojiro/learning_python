...
class Client():
    ...
    def get_file_by_name(self, filepath):
        ...
        for item in items:
            if item['filename'] == os.path.basename(filepath) :
                ...
                #print('    DEBUG: ref {0}'.format(item['filename']))
                #print('    DEBUG: tar {0}'.format(filepath))
                pass
        return res

    def add_file_to_knowledge(self, name, filename):
        k_id = self.get_knowledge_id(name)
        f_id = self.get_file_by_name(filename)
    ...
    def chat_completions_async(self, model, query, collection):
        collection_id = self.get_knowledge_id(collection)
        print('DEBUG: collection id is {0}'.format(collection_id))
    ...
    def chat_completions(self, model, query, collection):
