...
def generate_model(fp, data):
    ...
    # ★ ここで整数 PK を明示的に定義する
    # -------------------------------------------------
    fp.write("    id = models.AutoField(primary_key=True)\n")

    # -------------------------------------------------
    # フィールド生成（*_id はただの識別子として扱う）
    # -------------------------------------------------
    for fname, field_def in fields.items():
        ftype = field_def["type"]

        # 以前の primary_key 判定は廃止
        # *_id は単なる CharField / ForeignKey として扱う
        ...
        if ftype in ["ForeignKey", "OneToOneField"]:
            ...
            if not any(a.startswith("related_name=") for a in args):
                args.append(f"related_name='{name.lower()}s'")
            ...
            # primary_key は付けない（常に id が PK）
            arg_str = ", ".join(args)
            fp.write(f"    {fname} = models.{ftype}({arg_str})\n")

    # IPv4: address フィールドを追加（既存ロジック）
