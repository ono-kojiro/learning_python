...
@click.option("--export-installed-apps", "export_yaml", required=False,
              help="Export INSTALLED_APPS from settings.py to YAML")
@click.option("--replace-installed-apps", "replace_yaml", required=False,
              help="Replace INSTALLED_APPS in settings.py with YAML reference")
def installapp(app_name, project, export_yaml, replace_yaml):
    """Install an app or manage INSTALLED_APPS (export/replace)."""
    from amcli.commands.installapp import run
    run(
        app_name=app_name,
        project_dir=project,
        export_yaml=export_yaml,
        replace_yaml=replace_yaml,
    )
